...
const PostReviews = () => {
  ...
  // Submit form data (simulate or real submit)
  const submitForm = () => {
    if (  yesnodata) {
      const Data = {
  ...
  const handleContinue = () => {
    ...
    if (fduser === "success") {
      // User logged in, submit form and show success
      submitForm();
  ...
  // Listen to cookie changes or check periodically if fduser becomes "success" after modal login
  useEffect(() => {
    if (isModalOpen) {
      const interval = setInterval(() => {
        const fduser = Cookies.get("fduser");
        if (fduser === "success") {
          // Close modal and submit form automatically after login
          setIsModalOpen(false);
          submitForm();
